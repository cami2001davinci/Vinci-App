...
const CommentItem = ({ comment, depth = 0, onCommentChanged }) => {
  ...
  return (
    <div
      className="comment-item border rounded p-3 bg-white mb-2"
      style={{ marginLeft: depth * 16 }}
      data-comment-id={comment._id} // 👈 cartelito invisible con el ID
    >
...
const CommentsList = ({ postId, highlightId }) => {
  ...
  useEffect(() => {
    ...
    // esperamos al próximo “tick” para estar seguros de que el DOM ya se pintó
    const t = setTimeout(() => {
      const el = document.querySelector(`[data-comment-id="${highlightId}"]`);
      if (el) {
        el.scrollIntoView({ behavior: "smooth", block: "center" });
        el.classList.add("shadow", "bg-warning-subtle");
        setTimeout(() => {
          el.classList.remove("bg-warning-subtle", "shadow");
        }, 1800);
      }
    }, 50);
