...
export const flagComment = async (req, res) => {
  try {
    ...
    // 👇 FIX CLAVE: Si se reporta un raíz, ocultamos también sus respuestas
    if (!comment.parentComment) {
        await Comment.updateMany({ parentComment: comment._id }, { flagged: true });
    }
    ...
    try {
      ...
      const newCount = await CommentModel.countDocuments({
        post: comment.post,
        flagged: { $ne: true },
      });
      ...
      const payload = {
        ...
        action: "flagged",
        commentsCount: newCount // ENVIAMOS LA VERDAD ABSOLUTA
      };
...
export const createComment = async (req, res) => {
  try {
    ...
    if (!content || content.length < 1) {
      // Ajustado a 1 char mínimo, puedes poner 3 si prefieres
      return res
        .status(400)
...
export const getCommentPath = async (req, res) => {
  try {
    const commentId =
      req.params.pathId || req.params.commentId || req.params.id;
...
export const deleteComment = async (req, res) => {
  try {
    ...
    // 👇 LÓGICA TOMBSTONE (Soft Delete vs Hard Delete)
    if (comment.replies && comment.replies.length > 0) {
      // 1. Si tiene respuestas, NO lo borramos físicamente. Lo vaciamos (Lápida)
      comment.content = "[Comentario eliminado]";
      comment.isDeleted = true;
      await comment.save();
    } else {
      // 2. Si NO tiene respuestas, Hard Delete físico seguro
      hardDeleted = true;
      if (comment.parentComment) {
    ...
    try {
      ...
      // El conteo real ignora los flagged y las lápidas
      const CommentModel = mongoose.model("Comment");
      const newCount = await CommentModel.countDocuments({
        post: comment.post,
        flagged: { $ne: true },
        isDeleted: { $ne: true } // 👈 Ignoramos lápidas en el conteo
      });
      ...
      const payload = {
        ...
        hardDeleted // 👈 Le avisamos al frontend cómo reaccionar
      };

      emitToPost(comment.post, "comment:delete", payload);
      emitToAll("comment:delete", payload);
    } catch (err) {
      console.error("Error socket delete:", err.message);
    }

    res.json({ message: hardDeleted ? "Eliminado físicamente" : "Marcado como eliminado" });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
...
export const toggleLikeOnComment = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.error("Error like socket:", err.message);
    }
